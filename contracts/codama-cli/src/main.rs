...
// TODO
// //! Codama IDL build script with enhanced debugging.
// use {
//     codama::Codama,
//     std::{env, fs, path::Path},
// };
...
//     // Enhanced diagnostics: check the module structure
//     if lib_rs.exists() {
//         analyze_module_structure(&lib_rs)?;
//     }
...
// fn analyze_module_structure(lib_rs_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
//     let content = fs::read_to_string(lib_rs_path)?;
//     println!("Analyzing module structure...");

//     // Parse the file to analyze module declarations
...
//                     // Inline module
//                     println!("  Inline module with {} items", items.len());
//                     for item in items {
//                         if let syn::Item::Mod(sub_mod) = item {
//                             println!("    Submodule: {}", sub_mod.ident);
//                         }
//                     }
//                 } else {
//                     // External module - check if file exists
//                     let src_dir = lib_rs_path.parent().unwrap();
//                     let mod_file = src_dir.join(format!("{}.rs", module.ident));
//                     let mod_dir = src_dir.join(&module.ident.to_string()).join("mod.rs");
//                     let mod_dir_alt = src_dir.join(&module.ident.to_string());

//                     if mod_file.exists() {
//                         println!("  External module file: {}", mod_file.display());
//                     } else if mod_dir.exists() {
//                         println!("  External module dir with mod.rs: {}", mod_dir.display());
//                     } else if mod_dir_alt.is_dir() {
//                         println!("  External module directory: {}", mod_dir_alt.display());
//                         // Check what's inside the directory
//                         if let Ok(entries) = fs::read_dir(&mod_dir_alt) {
...
//                         }
//                     } else {
//                         println!("  ERROR: Module file not found for '{}'", module.ident);
//                         println!("    Looked for: {}", mod_file.display());
//                         println!("    Looked for: {}", mod_dir.display());
//                         println!("    Looked for: {}", mod_dir_alt.display());
//                     }
//                 }
//             }
//             _ => {}
...
fn main() {
    const TARGET_PATH: &str = "../packages/registry-cpi";
